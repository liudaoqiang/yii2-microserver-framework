 ...
 * @since 2.0
 */
class HanGreatLogger extends Target
{
    const LOG_IDENTIFIER = 'logIdentifier';
    const LOG_IDENTIFIER_TIMESTAMP = 'logIdentifierTimestamp';
    ...
    {
        return parent::getContextMessage(); // TODO: Change the autogenerated stub
    }
    ...
    public function getMessagePrefix($message)
    {
        ...
        $realIP = $request instanceof Request ? $request->getUserIP() : '-';
        ...
        return "[$realIP][$ip][$userID][$sessionID]";
    }
     ...
     * @return 无
     */
    public static function resetLogIdentifier()
    {
        $userId = 0;
//        if(!isset(Yii::$app->params['no_user_authentication']) || Yii::$app->params['no_user_authentication'] != 'yes') {
...
//        }
        ...
        $randNumber = rand();
        Yii::$app->session[self::LOG_IDENTIFIER] = "{$timePrefix}-u{$userId}-{$randNumber}";
        Yii::$app->session[self::LOG_IDENTIFIER_TIMESTAMP] = microtime(true);
    }

    /**
     * 记录日志（验证登录的）
     * @param $message string 日志消息体
     * @param int $level 日志等级
     * @param string $category 日志分类
     */
    public static function logWithIdentifier($message, $level = Logger::LEVEL_INFO, $category = 'ChinaHanGreat')
    {
        if (empty(Yii::$app->session[self::LOG_IDENTIFIER])) {
            self::resetLogIdentifier();
        }

        $logIdentifier = Yii::$app->session[self::LOG_IDENTIFIER];
        Yii::getLogger()->log('['.$logIdentifier.']'."\n".$message, $level, $category);
    }
     ...
     * @param string $category 日志分类
     */
    public static function log($message, $level = Logger::LEVEL_INFO, $category = 'ChinaHanGreat')
    {
        Yii::getLogger()->log("\n".$message, $level, $category);
    }
}
